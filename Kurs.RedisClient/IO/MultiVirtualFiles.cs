    ...
    : AbstractVirtualPathProviderBase, IVirtualFiles
{
    public MultiVirtualFiles(params IVirtualPathProvider[] childProviders)
    {
        ...
        ChildProviders = new List<IVirtualPathProvider>(childProviders);
        Initialize();
    }
    ...
    {
        return MultiVirtualDirectory.ToVirtualDirectory(ChildProviders.Select(p => p.GetDirectory(virtualPath))
            .Where(dir => dir != null));
    }
...
public class MultiVirtualDirectory : IVirtualDirectory
{
    ...
    {
        return dirs.SelectMany(dir => dir).GetEnumerator();
    }
    ...
    public bool IsRoot => dirs.First().IsRoot;
    ...
    public IEnumerable<IVirtualFile> GetAllMatchingFiles(string globPattern, int maxDepth = int.MaxValue)
    {
        foreach (var dir in dirs)
        {
