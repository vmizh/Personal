...
public static class EnumerableExtensions
{
    ...
    public static void Each<T>(this IEnumerable<T> values, Action<int, T> action)
    {
        ...
        var i = 0;
        foreach (var value in values) action(i++, value);
    }
    ...
    public static void Each<TKey, TValue>(this IDictionary<TKey, TValue> map, Action<TKey, TValue> action)
    {
        ...
        foreach (var key in keys) action(key, map[key]);
    }
    ...
    public static List<To> Map<To, From>(this IEnumerable<From> items, Func<From, To> converter)
    {
        ...
        foreach (var item in items) list.Add(converter(item));
        return list;
    }
        ...
        Func<T, T, bool> comparer = null)
    {
        ...
        foreach (var item in thisList)
        {
            ...
            if (thisIsDefault || otherIsDefault) return thisIsDefault && otherIsDefault;
    ...
    public static Dictionary<TKey, TValue> ToDictionary<T, TKey, TValue>(this IEnumerable<T> list,
        Func<T, KeyValuePair<TKey, TValue>> map)
    {
    ...
    public static T[] CombineDistinct<T>(this T[] original, params T[][] others)
    {
        ...
        foreach (var arr in others) count += arr.Length;

        var all = new List<T>(count);
        all.AddRange(original);
        foreach (var arr in others) all.AddRange(arr);
    ...
    public static HashSet<T> CombineSet<T>(this T[] original, params T[][] others)
    {
        ...
        foreach (var item in original) to.Add(item);
