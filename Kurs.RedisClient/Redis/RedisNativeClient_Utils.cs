...
public partial class RedisNativeClient
{
    ...
    private readonly List<ArraySegment<byte>> cmdBuffer = new List<ArraySegment<byte>>();
    ...
    private void Connect()
    {
        if (UsageTimer == null)
            ...
            if (!LicenseUtils.HasLicensedFeature(LicenseFeature.Redis))
                UsageTimer = new Timer(delegate { __requestsPerHour = 0; }, null, TimeSpan.FromMilliseconds(0),
        ...
        try
        {
            if (log.IsDebugEnabled)
            {
                ...
                logDebug(
                    $"Attempting {type} connection to '{Host}:{Port}' (SEND {SendTimeout}, RECV {ReceiveTimeout} Timeouts)...");
            }
            ...
            if (Ssl)
            {
                ...
                if (SslProtocols != null)
                    sslStream.AuthenticateAsClient(Host, new X509CertificateCollection(),
                        SslProtocols ?? System.Security.Authentication.SslProtocols.None, true);
                else
    ...
    protected void SendUnmanagedExpectSuccess(params byte[][] cmdWithBinaryArgs)
    {
        ...
        if (log.IsDebugEnabled && RedisConfig.EnableVerboseLogging)
            logDebug("stream.Write: " + Encoding.UTF8.GetString(bytes, 0, Math.Min(bytes.Length, 50))
                .Replace("\r\n", " ").SafeSubstring(0, 50));
        ...
        if (cmdBuffer.Count > 0)
        {
            ...
            if (!Env.IsMono && sslStream == null)
            {
                if (log.IsDebugEnabled && RedisConfig.EnableVerboseLogging)
                {
                    ...
                    logDebug("socket.Send: " +
                             StringBuilderCache.ReturnAndFree(sb.Replace("\r\n", " ")).SafeSubstring(0, 50));
                }
            ...
            else
            {
                ...
                foreach (var segment in cmdBuffer) SendDirectToSocket(segment);
            }
        }
    }
    ...
    {
        SendReceive<long>(cmdWithBinaryArgs, null, null,true);
    }
    ...
    {
        return SendReceive(cmdWithBinaryArgs, ReadLong, Pipeline != null ? Pipeline.CompleteLongQueuedCommand : null);
    }

    protected byte[] SendExpectData(params byte[][] cmdWithBinaryArgs)
    {
        return SendReceive(cmdWithBinaryArgs, ReadData, Pipeline != null ? Pipeline.CompleteBytesQueuedCommand : null);
    }

    protected double SendExpectDouble(params byte[][] cmdWithBinaryArgs)
    {
        return SendReceive(cmdWithBinaryArgs, ReadDouble,
            Pipeline != null ? Pipeline.CompleteDoubleQueuedCommand : null);
    }

    protected string SendExpectCode(params byte[][] cmdWithBinaryArgs)
    {
        return SendReceive(cmdWithBinaryArgs, ExpectCode,
            Pipeline != null ? Pipeline.CompleteStringQueuedCommand : null);
    }

    protected byte[][] SendExpectMultiData(params byte[][] cmdWithBinaryArgs)
    {
        return SendReceive(cmdWithBinaryArgs, ReadMultiData,
                   Pipeline != null ? Pipeline.CompleteMultiBytesQueuedCommand : null)
    ...
    protected RedisData SendExpectComplexResponse(params byte[][] cmdWithBinaryArgs)
    {
        return SendReceive(cmdWithBinaryArgs, ReadComplexResponse,
            Pipeline != null ? Pipeline.CompleteRedisDataQueuedCommand : null);
    }
            ...
            else
            {
                if (key == null)
                    throw new RedisResponseException(
                        "key == null, i={0}, data.Children[i] = {1}".Fmt(i, data.Children[i].ToRedisText().Dump()));
    ...
    protected void CmdLog(byte[][] args)
    {
        ...
        lastCommand = StringBuilderCache.ReturnAndFree(sb);
        if (lastCommand.Length > 100) lastCommand = lastCommand.Substring(0, 100) + "...";

        logDebug("S: " + lastCommand);
    }
    ...
    {
        var c = SafeReadByte(nameof(ExpectSuccess));
        if (c == -1)
    ...
    {
        var c = SafeReadByte(nameof(ExpectWord));
        if (c == -1)
            throw CreateNoMoreDataError();
    ...
    {
        var c = SafeReadByte(nameof(ExpectCode));
        if (c == -1)
            throw CreateNoMoreDataError();
    ...
    {
        var c = SafeReadByte(nameof(ReadLong));
        if (c == -1)
            throw CreateNoMoreDataError();
    ...
    {
        var c = SafeReadByte(nameof(ReadMultiData));
        if (c == -1)
            throw CreateNoMoreDataError();
    ...
    {
        var c = SafeReadByte(nameof(ReadDeeplyNestedMultiDataItem));
        if (c == -1)
            throw CreateNoMoreDataError();
    ...
    {
        var c = SafeReadByte(nameof(ReadComplexResponse));
        if (c == -1)
            throw CreateNoMoreDataError();
        ...
        switch (c)
        {
            ...
            case '*':
                if (int.TryParse(s, out var count))
                {
                    ...
                    for (var i = 0; i < count; i++) ret.Children.Add(ReadComplexResponse());
    ...
    {
        var c = SafeReadByte(nameof(ReadMultiDataResultCount));
        if (c == -1)
            throw CreateNoMoreDataError();
