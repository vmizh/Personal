...
public static class TypeConfig<T>
{
    ...
    private static TypeConfig Config => config ??= Create();
    ...
    private static TypeConfig Create()
    {
        ...
        if (!JsConfig<T>.HasDeserializingFn)
            ...
            config.OnDeserializing = (instance, memberName, value) =>
                JsConfig<T>.OnDeserializingFn((T)instance, memberName, value);
