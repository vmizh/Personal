...
public sealed class BufferPool
{
    ...
    public static byte[] GetCachedBuffer(int minSize)
    {
        lock (Pool)
        {
            ...
            {
                var buffer = Pool[i];
                if (buffer == null || buffer.Size < minSize) continue;
                if (bestMatch != null && bestMatch.Length < buffer.Size) continue;
            ...
            if (bestIndex >= 0) Pool[bestIndex] = null;
    ...
    public static void ResizeAndFlushLeft(ref byte[] buffer, int toFitAtLeastBytes, int copyFromIndex, int copyBytes)
    {
        ...
        if (copyBytes == 0) ReleaseBufferToPool(ref buffer);
