...
public static class JsWriter
{
    ...
    static JsWriter()
    {
        foreach (var escapeChar in EscapeChars) EscapeCharFlags[escapeChar] = true;
    ...
    where TSerializer : ITypeSerializer
{
    ...
    public WriteObjectDelegate GetValueTypeToStringMethod(Type type)
    {
        ...
        {
            if (underlyingType.IsEnum) return Serializer.WriteEnum;
        }
    ...
    {
        if (typeof(T) == typeof(string)) return Serializer.WriteObjectString;
        ...
        if (JsConfig<T>.HasSerializeFn) ret = JsConfig<T>.WriteFn<TSerializer>;

        if (ret == null) ret = GetCoreWriteFn<T>();
    ...
    {
        if (typeof(T).IsInstanceOf(typeof(Task)))
        ...
        if (specialWriteFn != null) return specialWriteFn;
        ...
        var isDictionary = typeof(T) != typeof(IEnumerable) && typeof(T) != typeof(ICollection)
                                                            && (typeof(T).IsAssignableFrom(typeof(IDictionary)) ||
                                                                typeof(T).HasInterface(typeof(IDictionary)));
        if (isDictionary) return WriteDictionary<TSerializer>.WriteIDictionary;

        var isEnumerable = typeof(T).IsAssignableFrom(typeof(IEnumerable))
                           || typeof(T).HasInterface(typeof(IEnumerable));
        if (isEnumerable) return WriteListsOfElements<TSerializer>.WriteIEnumerable;
        ...
        if (typeof(T).IsClass || typeof(T).IsInterface || JsConfig.TreatAsRefType(typeof(T)))
        {
            ...
            if (typeToStringMethod != null) return typeToStringMethod;
        }
