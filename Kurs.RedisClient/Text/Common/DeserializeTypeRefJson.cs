...
internal static class DeserializeTypeRefJson
{
        ...
        KeyValuePair<string, TypeAccessor>[] typeAccessors)
    {
        ...
        {
            var result = JsonTypeSerializer.UnescapeJsString(strType, JsonUtils.QuoteChar, true, index);
            ...
            if (instance == null && possibleTypeInfo &&
                propertyName.Equals(typeAttr.Span, StringComparison.OrdinalIgnoreCase))
            {
            ...
            var propType = possibleTypeInfo && propertyValueStr[0] == '_'
            ...
            if (propType != null)
                try
                {
                    if (typeAccessor != null)
                    {
                        ...
                        if (typeConfig.OnDeserializing != null)
                            propertyValue =
                                typeConfig.OnDeserializing(instance, propertyName.ToString(), propertyValue);
                ...
                {
                    config.OnDeserializationError?.Invoke(instance, propType, propertyName.ToString(),
                        propertyValueStr.ToString(), e);
                    if (config.ThrowOnError)
                        throw DeserializeTypeRef.GetSerializationException(propertyName.ToString(),
                            propertyValueStr.ToString(), propType, e);
                    Tracer.Instance.WriteWarning("WARN: failed to set dynamic property {0} with: {1}",
            ...
            if (typeAccessor?.GetProperty != null && typeAccessor.SetProperty != null)
                ...
                {
                    config.OnDeserializationError?.Invoke(instance, propType ?? typeAccessor.PropertyType,
                        propertyName.ToString(), propertyValueStr.ToString(), e);
                    if (config.ThrowOnError)
                        throw DeserializeTypeRef.GetSerializationException(propertyName.ToString(),
                            propertyValueStr.ToString(), typeAccessor.PropertyType, e);
                    Tracer.Instance.WriteWarning("WARN: failed to set property {0} with: {1}", propertyName.ToString(),
                        propertyValueStr.ToString());
                }
            else
