...
{
    public delegate object ParseListDelegate(ReadOnlySpan<char> value, Type createListType,
        ParseStringSpanDelegate parseFn);

    internal static readonly ITypeSerializer Serializer = JsWriter.GetTypeSerializer<TSerializer>();

    private static Dictionary<Type, ParseListDelegate> ParseDelegateCache = new Dictionary<Type, ParseListDelegate>();

    private static readonly Type[] signature =
        { typeof(ReadOnlySpan<char>), typeof(Type), typeof(ParseStringSpanDelegate) };
    ...
    {
        return ParseIntList(value.AsSpan());
    }
    ...
    {
        return ParseByteList(value.AsSpan());
    }
    ...
    where TSerializer : ITypeSerializer
{
    ...
    public static ICollection<T> ParseGenericList(ReadOnlySpan<char> value, Type createListType,
        ParseStringSpanDelegate parseFn)
    {
        var isReadOnly = createListType != null && createListType.IsGenericType &&
                         createListType.GetGenericTypeDefinition() == typeof(ReadOnlyCollection<>);
        var to = createListType == null || isReadOnly
            ? new List<T>()
            : (ICollection<T>)createListType.CreateInstance();

        var objSerializer = JsonTypeSerializer.Instance.ObjectDeserializer;
        if (to is List<object> && objSerializer != null && typeof(TSerializer) == typeof(JsonTypeSerializer))
                ...
                while (i < valueLength)
                {
                    ...
                    if (!isEmpty)
                    {
                        if (tryToParseItemsAsPrimitiveTypes)
                        {
                            ...
                            to.Add((T)DeserializeType<TSerializer>.ParsePrimitive(elementValue.Value(),
                                value[startIndex]));
                        }
                        else
                        {
    ...
    where TSerializer : ITypeSerializer
{
    ...
    {
        ParseStringSpan = GetParseStringSpanFn();
    }

    public static ParseStringDelegate Parse => v => ParseStringSpan(v.AsSpan());

    public static ParseStringSpanDelegate ParseStringSpan { get; }

    public static ParseStringDelegate GetParseFn()
    {
        return v => GetParseStringSpanFn()(v.AsSpan());
    }
    ...
    public static ParseStringSpanDelegate GetParseStringSpanFn()
    {
        ...
        var supportedTypeParseMethod =
            DeserializeListWithElements<TSerializer>.Serializer.GetParseStringSpanFn(elementType);
        if (supportedTypeParseMethod != null)
        {
            ...
            var parseFn =
                DeserializeListWithElements<TSerializer>.GetListTypeParseStringSpanFn(createListType, elementType,
                    supportedTypeParseMethod);
