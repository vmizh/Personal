    ...
    where TSerializer : ITypeSerializer
{
    ...
    {
        return ParseGenericArray(value.AsSpan(), v => elementParseFn(v.ToString()));
    }
...
internal static class DeserializeArray<TSerializer>
    where TSerializer : ITypeSerializer
{
    private static Dictionary<Type, ParseStringSpanDelegate> ParseDelegateCache =
        new Dictionary<Type, ParseStringSpanDelegate>();
